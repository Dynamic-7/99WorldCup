...
`npm run build` produces one self-contained file, which is the right artifact
for publishing as a Claude artifact but isn't how you'd hand a folder to a
static host. For that, use **`99WorldCup-Vercel/`** — a sibling folder, next
to this project, containing the same `src/` with nothing rewritten by hand:
...
No `package.json`, no `node_modules`, no build step. Drag that folder into
Vercel (or point any static file server at it) and it runs. It has no
connection to the Claude artifact host, so `App.online` is `false` and it
plays as a single-player cabinet — see **Known limitations → Leaderboard
persistence** below for where a real backend plugs in.
