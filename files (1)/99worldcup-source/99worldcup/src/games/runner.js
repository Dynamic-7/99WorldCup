...
export function Runner(o){
  ...
  /* ---------------------------------------------------------------- layout */
  /* Desktop: fit the glass to available vertical space, same as before, so a
     tall game never pushes its own controls off the bottom of the window.
     Phone/small tablet: width is what was scarce, not height — the screen now
     simply fills the bezel edge to edge (a plain block element is 100% of its
     parent by default, so clearing any inline max-width is enough) and the
     page scrolls to reach the deck below. Aspect ratio still comes from the
     canvas's own width:100%/height:auto rule, so nothing is stretched. */
  const MOBILE_BP = 640;
  function fitScreen(){
    if (!game || !o.screen) return;
    if (window.innerWidth < MOBILE_BP){ o.screen.style.maxWidth = ''; return; }
